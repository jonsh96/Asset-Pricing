function [t_lambda, lambda, alpha, beta, gamma, covariance, dates, excess_returns] = run_regression(startDate, endDate, data, risk_free_data, excess_return_data, market_data, monthly_data, liquidity_data, factorIndex)
    % Data can be AVWR or any other table specified in the main.m

    % Matching the indices to the tables
    [startIndex, endIndex] = index_tables(startDate, endDate, market_data, monthly_data, liquidity_data);
    ...
    % If the factor indices are empty, i.e. 0, then we only consider
    % excess_returns 
    if factorIndex == 0
